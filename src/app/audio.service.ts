...
export class AudioService {
  ...
   /**
    ...
    * @param audio
    */
   public Toggle(audio: MusicItem): void {
      const tryGet = audio ?
         this.playList.findIndex((p) => p.file === audio.file) :
         this.playData.Index;
      if (tryGet < 0) {
  ...
   /**
    ...
    * @param audio
    */
   public Add(audio: MusicItem): void {
      this.playList.push(audio);
   ...
   * 播放指定索引的音频
   */
  public currentPlay(id: number): void {
    this.PlayIndex(id);
  }
  ...
   private PlayIndex(index: number): void {
    ...
      if (index !== -1) {
         this._audio.src = this.playList[index].file;
         if (this._audio.paused) {
        this._audio.play();
            this.playData.IsPlaying = true;
         }
